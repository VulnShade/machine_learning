...
def orthogonal_projection(p, n):
    """
    Computes the orthogonal projection of point p onto the plane defined by normal n.
    """
    n = np.array(n)
    p = np.array(p)
...
def orthogonal_set_projection(P, n):
    """
    Computes the orthogonal projection of each point in P onto the plane defined by n.
    """
    l = []
    for p in P:
...
def rotation_matrix(angle):
    ...
    return np.matrix([[np.cos(radians), -np.sin(radians)],
                      [np.sin(radians),  np.cos(radians)]], dtype=float).round(4)
...
def plot_widget_2(doc):
    ...
    # PCA line
    pca_line_plot = line_between_points(main_plot, pca_line[0], pca_line[1],
                                        color='#333333', line_width=1.5, legend_label="PCA Line")
    ...
    rotating_line = line_between_points(main_plot,
                                        ...
                                        color='#0096FF', line_width=1.5)
    projection_points = main_plot.scatter(orthogonal_set[:,0],
                                          orthogonal_set[:,1],
                                          marker='x', size=10, color="#C00000",
    ...
    rhs_scatter = rhs_plot.scatter(rhs_plot_points[:,0], rhs_plot_points[:,1],
                                   color="#C00000", size=5, marker="x")
    ...
    slider = Slider(title="Adjust rotation angle", start=0, end=360, value=0, step=1)
...
def plot_3d_2d_graphs(X):
    ...
    fig.add_trace(go.Scatter3d(x=df['x1'], y=df['x2'], z=df['x3'], mode='markers'),
                  row=1, col=1
...
def plot_widget():
    ...
    rotating_line = line_between_points(px, rotating_line_initial_state[0], rotating_line_initial_state[1])
    ...
    # 1D projection (right-hand side plot)
    ...
    # Combine data for the main figure
    ...
    # Get initial points from the rotating line
    ...
    # Define figure widgets BEFORE update() so they are in scope.
    fig = go.FigureWidget(data=final_data).update_yaxes(scaleanchor='x', scaleratio=1, range=[-1,1], visible=False)\
                                     .update_xaxes(range=[-1.5,1.5], visible=False)
    rhs_fig = go.FigureWidget(data=rhs_line.data + rhs_scatter.data)\
                    .update_yaxes(scaleanchor='x', scaleratio=1, range=[-1,1], showgrid=False, visible=False)\
                    .update_xaxes(range=[-1.5,1.5], showgrid=False, visible=False)
    ...
    # Define update function after fig and rhs_fig exist.
    def update(angle):
        ang = angle
        with fig.batch_update():
            ...
            if abs(p0r[0] - p1r[0]) < 1e-10:
                ...
                n_line = (-(p0r[1]-p1r[1])/(p0r[0]-p1r[0]), 1)
            orthogonal_to_line = orthogonal_set_projection(X, n_line)
            # Update projection lines for each point
            for i, (orig, proj) in enumerate(zip(X, orthogonal_to_line)):
                fig.data[i]['x'] = [orig[0], proj[0]]
                fig.data[i]['y'] = [orig[1], proj[1]]
            fig.data[-2]['x'] = [p0r[0], p1r[0]]
            fig.data[-2]['y'] = [p0r[1], p1r[1]]
            fig.data[-1]['x'] = orthogonal_to_line[:,0]
            fig.data[-1]['y'] = orthogonal_to_line[:,1]
            projection_plot_1d = get_plane_base(orthogonal_to_line, n_line)
            rhs_fig.data[1]['x'] = projection_plot_1d[:,0]
